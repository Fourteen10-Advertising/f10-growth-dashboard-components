...
/** Map a thrown pipeline error to a stable log outcome code (US-008). Every
 * failure path throws an error carrying a status (or a guard `code`); this turns
 * that into the small, queryable vocabulary the log's `outcome` column uses. */
function outcomeFor(err) {
  if (err) {
    if (err.code === 'UNSAFE_SQL') return 'unsafe_sql';
    if (err.code === 'OUT_OF_SCOPE_TABLE') return 'out_of_scope_table';
    switch (err.status) {
      case 400: return 'bad_request';
      case 413: return 'too_much_data';
      case 422: return 'cannot_answer';
...
async function runAsk({ model, question, today, clients, logger, defaultDateRange }) {
  ...
  // Hoisted above the try so the failure logger in the catch can record which
  // resolution path we reached and the SQL that failed, even when a stage throws
  // partway through the pipeline.
  let path = null, sql = null;
  try {
    ...
    let spec = null, built = null, fallbackViz = null;
    ...
    // A failed ask is the highest-value demand signal — the client asked
    // something the dashboard could not answer. Log it (outcome != 'ok') before
    // rethrowing, so the handler still maps status/message exactly as before.
    // safe() so a logging failure never masks the original error.
    if (logger) await safe(() => logger({
      question,
      path,
      sql,
      bytesProcessed: 0,
      ...
      error: (err && (err.cause || err.message)) || String(err),
    }));
    throw err;
  }
}
